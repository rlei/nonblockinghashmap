...
impl<K: Hash, V> KVs<K, V>{
		...
		KVs {
			_ks: {
				...
				for _ in 0..table_size {
					temp.push(AtomicPtr::new(Box::into_raw(Box::new(Key::<K>::new_empty()))));
				}
				temp
			},
			_vs: {
				let mut temp = Vec::with_capacity(table_size);
				for _ in 0..table_size {
					temp.push(AtomicPtr::new(Box::into_raw(Box::new(Value::<V>::new_empty()))));
				}
				temp
			},
			_chm: CHM::<K, V>::new(),
	...
	pub fn table_full(&self, reprobe_cnt: usize) -> bool{
		reprobe_cnt >= REPROBE_LIMIT &&
			self._chm._slots.load(Ordering::SeqCst) >= self._ks.len()
	}
