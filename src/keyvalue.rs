...
impl<T: Hash> Key<T> {
	pub fn new(k: T) -> Key<T> {
		Key { _keytype: KeyTypes::KeyType, _key: Box::into_raw(Box::new(k)) }
	}
	...
	pub fn new_empty() -> Key<T> {
		Key { _keytype: KeyTypes::KeyEmpty, _key: ptr::null_mut() }
	}

	pub fn new_tombstone() -> Key<T> {
		Key { _keytype: KeyTypes::KeyTombStone, _key: ptr::null_mut() }
	}
...
impl<T: PartialEq + Hash> PartialEq for Key<T>{
	fn eq(&self, other: &Key<T>) -> bool{
		...
		if self._keytype == KeyTypes::KeyEmpty && other._keytype == KeyTypes::KeyEmpty { return true; } 
		if self._keytype == KeyTypes::KeyTombStone && other._keytype == KeyTypes::KeyTombStone { return true; }
...
impl<T> Value<T> {
	pub fn new(v: T) -> Value<T> {
		Value { _valuetype: ValueTypes::ValueType, _value: Box::into_raw(Box::new(v)), _is_prime: false }
	}

	pub fn new_empty() -> Value<T> {
		Value { _valuetype: ValueTypes::ValueEmpty, _value: ptr::null_mut(), _is_prime: false }
	}

	pub fn new_tombstone() -> Value<T> {
		Value { _valuetype: ValueTypes::ValueTombStone, _value: ptr::null_mut(), _is_prime: false }
	}

	pub fn new_tombprime() -> Value<T> {
		Value { _valuetype: ValueTypes::ValueTombStone, _value: ptr::null_mut(), _is_prime: true }
	}

	pub fn new_prime(v: T) -> Value<T> {
		Value { _valuetype: ValueTypes::ValueType, _value: Box::into_raw(Box::new(v)), _is_prime: true }
	}
	...
	pub fn get_prime(&self) -> *mut Value<T>{
		...
		Box::into_raw(Box::new(Value { _valuetype: self._valuetype, _value: self._value, _is_prime: true }))
	}

	pub fn get_unprime(&self) -> *mut Value<T>{
		assert!(self.is_prime());
		Box::into_raw(Box::new(Value { _valuetype: self._valuetype, _value: self._value, _is_prime: false }))
	}
...
impl<T: PartialEq> PartialEq for Value<T>{
	fn eq(&self, other: &Value<T>) -> bool{
		...
		if self._valuetype==ValueTypes::ValueEmpty && other._valuetype==ValueTypes::ValueEmpty { return true; } 
		if self._valuetype==ValueTypes::ValueTombStone && other._valuetype==ValueTypes::ValueTombStone && self._is_prime==other._is_prime { return true; }
