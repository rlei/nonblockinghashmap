    ...
    pub fn copy_slot(&mut self, oldkvs: *mut KVs<K, V>, idx: usize) -> bool {
        unsafe {
            ...
            while !(*oldvalue).is_prime() {
                ...
                if (*oldkvs)._vs[idx].compare_and_swap(oldvalue, primed, MEMORY_ORDERING)
                    == oldvalue
                {
    ...
    unsafe {
        match (*value).valuetype() {
            ...
            ValueType => {
                ...
                if (*value).is_prime() {
                    String::from("Prime(") + &(*(*value)._value).to_string() + ")"
                } else {
