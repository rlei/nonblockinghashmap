...
use keyvalue::{Key, Value, KeyTypes::KeyTombStone, ValueTypes::ValueTombStone, KeyTypes::KeyType, ValueTypes::ValueType, KeyTypes::KeyEmpty, ValueTypes::ValueEmpty};
...
impl<K: Eq + Hash,V: Eq> NonBlockingHashMap<K,V> {
	...
	pub fn resize(&self, kvs: *mut KVs<K,V>) -> *mut KVs<K,V> {
		unsafe {
			...
			let tm = Instant::now();
			if newsz <= oldlen && tm.duration_since(self._last_resize) <= Duration::new(1, 0) && (*kvs)._chm._slots.load(MEMORY_ORDERING) >= sz<<1 {
				newsz = oldlen<<1;			
			}
	...
	pub fn put<'a>(&mut self, key: K, newval: V) -> &'a V{
		self.put_if_match(key, newval, MatchingTypes::MatchAll, None)
	}
	...
	pub fn put_if_match_to_kvs<'a>(&mut self, kvs: *mut KVs<K,V>, key: K, newval: V, matchingtype: MatchingTypes, expval: Option<V>) -> &'a V{
		let new_expval =
			expval.map(|v| Box::into_raw(Box::new(Value::<V>::new(v))));
		let returnval = self.put_if_match_impl(kvs, Box::into_raw(Box::new(Key::<K>::new(key))), Box::into_raw(Box::new(Value::<V>::new(newval))), matchingtype, new_expval);
		return unsafe{ &(*(*returnval)._value) };
	}
		...
		unsafe {
			...
			loop {
				...
				if matchingtype!=MatchingTypes::MatchAll && // If expval is not a wildcard
					!( matchingtype==MatchingTypes::MatchAllNotEmpty && !(*v).is_tombstone() && !(*v).is_empty() ) // If expval is not a TombStone or Empty
					{
				...
				if (*kvs)._vs[idx].compare_and_swap(v, putval, MEMORY_ORDERING)==v {
					...
					if (*v).is_empty() && expval_not_empty { return Box::into_raw(Box::new(Value::<V>::new_tombstone())) }
					else { return v; }
				}
				v = (*kvs).get_value_nonatomic_at(idx);
				if (*v).is_prime(){
	...
	pub fn get<'a>(&'a mut self, key: K) -> Option<&'a V>{
		let table = self.get_table_nonatomic();
		let returnvalue = self.get_impl(table, Box::into_raw(Box::new(Key::<K>::new(key))));
		if returnvalue.is_some() {
			return Some(unsafe { &*(*returnvalue.unwrap())._value });
		}
		else { return None; }
	}
	...
	pub fn copy_slot_and_check(&mut self, oldkvs: *mut KVs<K,V>, idx: usize, should_help: bool) -> *mut KVs<K,V>{
		//fence(MEMORY_ORDERING);
		unsafe {
	...
	pub fn copy_check_and_promote(&mut self, oldkvs: *mut KVs<K,V>, work_done: usize){
		unsafe {
	...
	pub fn copy_slot(&mut self, oldkvs: *mut KVs<K,V>, idx: usize) -> bool{
		unsafe {
			...
			self.put_if_match_impl(newkvs, key, old_unprimed, MatchingTypes::MatchValue, Some( emptyval ));
	...
	pub fn get_kvs_level_impl(kvs: *mut KVs<K,V>, level: u32) -> Option<*mut KVs<K,V>>{
		if kvs.is_null() { return None; }
		if level==0 { 
			Some(kvs) 
		} else {
			unsafe{ NonBlockingHashMap::get_kvs_level_impl((*kvs)._chm.get_newkvs_nonatomic(), level-1) }
		}
	}
...
// debuging functions
pub fn print_table<K: Eq + Hash + ToString, V: Eq + ToString>(table: &NonBlockingHashMap<K,V>){
...
pub fn print_kvs<K: Eq + Hash + ToString,V: Eq + ToString>(kvs: *mut KVs<K,V>){
	unsafe {
		for i in 0..(*kvs).len() {
...
pub fn key_to_string<K: Eq + Hash + ToString>(key: *mut Key<K>) -> String { 
	unsafe {
		match (*key).keytype() {
...
pub fn value_to_string<V: Eq + ToString>(value: *mut Value<V>) -> String {
	unsafe {
		match (*value).valuetype() {
			ValueTombStone => {
				if (*value).is_prime() { String::from("TOMBPRIME") }
				else { String::from("TOMBSTONE") }
			}
			ValueEmpty => { String::from("EMPTY") }
			ValueType => { 
				assert!(!(*value)._value.is_null());
				if (*value).is_prime() { String::from("Prime(") + &(*(*value)._value).to_string() + ")" }
				else { (*(*value)._value).to_string() }
			}
		}
	}
}
...
mod test {
	...
	fn test_value_eq(){
		unsafe {
			...
			assert!((*Value::<i32>::new_tombstone().get_prime())==(*Value::<i32>::new_tombstone().get_prime()));
	...
	fn test_hashmap_init(){
		...
		assert!(map.len() == 16*4);
		unsafe {
			assert!((*map._kvs.load(MEMORY_ORDERING))._chm._newkvs.load(MEMORY_ORDERING).is_null());
		}
	}
	...
	fn test_hashmap_resize(){
		...
		map1.resize(kvs);
		unsafe {
			assert_eq!((*(*kvs)._chm._newkvs.load(MEMORY_ORDERING)).len(), 16*4*2);
		}
		let kvs = unsafe {(*kvs)._chm._newkvs.load(MEMORY_ORDERING)};
		map1.resize(kvs);
		unsafe {
			assert_eq!((*(*kvs)._chm._newkvs.load(MEMORY_ORDERING)).len(), 16*4*4);
		}
